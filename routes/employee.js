...
router.post("/", function (req, res) {
  ...
  .then(data => {
    if(data && req.body.phone) {
      return messaging.send({msgTo: req.body.phone, body: `Here's link: ${getLink(req)}`})
      .then(result => res.send({message: 'Check your phone', forTesting: getLink(req)}))
    } else {
      return res.status(400).json({message: 'Phone # not valid:' + req.body.phone})
    }
  })
  .catch(err => res.status(500).send({error: err.message, stack: err.stack}))
})
